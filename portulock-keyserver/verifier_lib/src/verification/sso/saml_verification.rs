...
impl SamlVerifier {
    ...
    pub async fn new(
        ...
        let sp_metadata = service_provider.metadata()?.to_xml()?;
    ...
    pub fn get_auth_url(&self) -> Result<(Url, AuthChallengeData), VerifierError> {
        let auth_request = self.sp.make_authentication_request(&self.idp_url)?;
        let auth_url = auth_request.redirect("")?.ok_or("Failed to generate AuthURL!")?;
        println!(
    ...
    pub async fn verify_and_extract_claims(
        ...
        let assertion = self.sp.parse_response(auth_response, &[auth_challenge.request_id])?;
...
impl TryFrom<AuthChallengeData> for SAMLAuthChallenge {
    ...
    fn try_from(value: AuthChallengeData) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            request_id: value
                .get("request_id")
                .ok_or("Missing request_id in saml AuthChallenge!")?
                .to_string(),
        })
    }
}
